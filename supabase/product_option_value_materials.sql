-- Optional link from product option values to materials library (product_materials).
-- Requires product_materials.sql first. Safe to run more than once.
-- Does not change checkout logic.
...
create index if not exists product_option_values_material_id_idx
  on public.product_option_values (material_id);

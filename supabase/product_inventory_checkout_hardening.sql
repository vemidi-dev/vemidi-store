-- Product inventory checkout hardening (migration #36).
-- Run after product_inventory_fulfillment.sql (#35).
-- Safe to re-run: CREATE OR REPLACE only.
--
-- Fixes:
--   - Lock every ordered product with FOR UPDATE (stable product_id order).
--   - Check is_sold_out before unavailable/stock validation.
--   - Return product_sold_out when is_sold_out=true even if stock is low.
--   - Aggregate cart demand before stock checks (unchanged from #35).

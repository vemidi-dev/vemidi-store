-- Product-level promo code eligibility for discount coupons.
-- Run on existing Supabase AFTER discount_coupons.sql baseline.
-- Safe to re-run. Do not run automatically — apply manually in Supabase SQL editor.
--
-- Fresh setups: also covered by discount_coupons.sql (column + create_store_order).

begin;

alter table public.products
  add column if not exists promo_code_eligible boolean not null default true;

comment on column public.products.promo_code_eligible is
  'When true, percentage discount coupons may apply to this product line total.';
...
declare
  ...
  v_coupon record;
  v_promo_code_eligible boolean := true;
begin
  ...
  loop
    ...
    select
      ...
      status,
      visibility,
      coalesce(promo_code_eligible, true) as promo_code_eligible
      into v_product
      from public.products
    ...
    v_promo_code_eligible := coalesce(v_product.promo_code_eligible, true);
    if v_promo_code_eligible then
    ...
    v_items := v_items || jsonb_build_array(
      jsonb_build_object(
        ...
        'promoCodeEligible', v_promo_code_eligible,
        'stockQuantityBefore', v_stock_before,
        'stockQuantityAfter', v_stock_after,
  ...
  if nullif(trim(coalesce(p_coupon_code, '')), '') is not null then
    ...
    -- Percent coupons apply only to promo-code-eligible line totals.
    v_discount_amount := round(v_eligible_subtotal * v_discount_percentage / 100.0, 2);
    v_total := greatest(0, v_subtotal - v_discount_amount);
    -- One-time codes are marked used only when a real discount was granted.
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'order', jsonb_build_object(
        ...
        'eligibleSubtotalPrice', v_eligible_subtotal,

-- Time-bound product promotions with server-side effective pricing.
-- Run in Supabase SQL Editor after products_table.sql and store_checkout_orders.sql.
...
create table if not exists public.product_promotions (
  ...
  constraint product_promotions_name_check check (char_length(btrim(name)) >= 1),
  constraint product_promotions_discount_type_check check (
    discount_type in ('percentage', 'fixed_price')
  ),
  constraint product_promotions_discount_value_check check (discount_value >= 0),
  constraint product_promotions_period_check check (ends_at > starts_at),
  constraint product_promotions_percentage_check check (
    discount_type <> 'percentage'
    or (discount_value > 0 and discount_value <= 100)
  )
);

create index if not exists product_promotions_product_period_idx
  on public.product_promotions (product_id, starts_at desc, ends_at desc);

create index if not exists product_promotions_active_period_idx
  on public.product_promotions (product_id, starts_at, ends_at)
  where is_active;

alter table public.product_promotions
  drop constraint if exists product_promotions_no_overlap;

alter table public.product_promotions
  add constraint product_promotions_no_overlap
  exclude using gist (
    product_id with =,
...
create policy "product_promotions_public_read"
on public.product_promotions
for select
to anon, authenticated
using (true);

drop policy if exists "product_promotions_admin_write" on public.product_promotions;
create policy "product_promotions_admin_write"
on public.product_promotions
for all
to authenticated
...
grant execute on function public.resolve_product_unit_price(uuid, timestamptz) to anon, authenticated, service_role;

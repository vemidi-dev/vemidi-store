...
export const siteContentDefaults = {
  ...
  "terms.pricing_text":
    "Цените са обявени в евро (EUR). Курирската доставка не е включена, освен ако изрично не е посочено друго. Плащането е с наложен платеж при получаване. Промо кодовете за отстъпка не се прилагат за продукти, за които е посочено, че са изключени от промо кодове, включително заготовки и материали. За тях могат да важат отделни условия за намаление според броя закупени артикули. Ако поръчката съдържа както продукти с право на промо код, така и продукти без право на промо код, отстъпката се прилага само към допустимите продукти.",
